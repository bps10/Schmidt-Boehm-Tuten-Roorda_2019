...
ggplot(data=sessionMerge, aes(x=predictedGR, y=gr12)) + 
  geom_point() +
...
unity = data.frame(x=c(-1, 1), y=c(-1, 1))
ggplot(data=sessionMerge, aes(x=predictedYB, y=yb12)) + 
  geom_point() +
  geom_smooth(method=lm) +
  coord_equal(xlim=c(-1, 1), ylim=c(-1, 1)) +
  geom_path(data=unity, aes(x=x, y=y), colour='gray', linetype=1) +
...
mod <- lm(predictedGR ~ gr12, sessionMerge)
summary.lm(mod)

mod <- lm(predictedYB ~ yb12, sessionMerge)
summary.lm(mod)
```
